...
# TODO move GIS utility functions into separate module to be called by mining and environment ?
...
def combine(rasterFunction, selectLayers, gisFilename, classifiedFilename, outputFilename):
    ...
    resultRasterDataset = createEmptyCopy(outputFilename, classifiedDataset)

    # apply function
    rasterFunction(gisRasterLayersDataset, classifiedDataset, resultRasterDataset)

    # close result dataset
    resultRasterDataset = None

def cropSourceToDest(sourceDataset, destDataset):

    """
    Crop a source dataset to the same dimensions and format as a raster destination dataset.
    """
...
def createEmptyCopy(filename, inputDataset):

    """
    Return an empty dataset with the given filename and the same metadata as the input dataset.
    """
    ...
    outputDataset = driver.create(outputFilename, xSize, ySize, layerCount, dataType)

    # copy metadata from input dataset
