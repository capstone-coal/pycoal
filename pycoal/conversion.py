...
class USGSSpectral7ToAsterConversion:
    ...
    def convert(cls, library_filename=""):
        ...
        with open(library_filename, 'r') as input_file:
            ...
            k = 0
            # Loop through file and store all wavelength values for the
            # given Spectra
            with open('SpectraValues.txt', 'w') as spectra_values_file:
                ...
                while k < line_count:
                    ...
                    spectra_wave_length = float(
                        "{0:.5f}".format(spectra_wave_length))
                    spectra_y_value = spectra_wave_length * 10
                    line = str(spectra_wave_length) + '  ' + str(
                        spectra_y_value)
        ...
        # using stored Spectra Name and stored Spectra Wavelength values`
        with open(library_filename, 'w') as input_file:
            ...
            input_file.write('Origin: Spectra obtained from the '
                             'Noncoventional Exploitation Factors\n')
            input_file.write('Data System of the National Photographic '
                             'Interpretation Center.\n')
            input_file.write('Description:  Gray and black construction '
                             'asphalt.  The sample was\n')
            input_file.write('soiled and weathered, with some limestone and '
                             'quartz aggregate\n')
            ...
            input_file.write('\n')
            j = 0
            # Read in values saved in SpectraValues.txt and output them to
            # the library_filename
...
class FullUSGSSpectral7ToENVIConversion:
    ...
    def convert(cls, library_filename=""):
        ...
        convolved envi format
            ...
            Args: library_filename (str): path to USGS Spectral Library
            Version 7 directory
        """
        if not library_filename:
            raise ValueError(
